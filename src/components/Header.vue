<template>
  <v-app-bar app color="white" elevation="0" clipped-left height="80px" absolute>
    ...
    <div class="d-flex align-center" v-if="!$isLoggedIn">
      <v-btn outlined text syle="color: #2E2D29; font-size: 105%" class="rambla-font" :to="{name: 'register'}">
        Join Us
      </v-btn>
      <h2 style="margin-right: 10px; margin-left: 10px; color: #2E2D29">
        |
      </h2>
      <v-btn outlined text syle="color: #2E2D29; font-size: 105%" class="rambla-font" :to="{name: 'login'}">
        Log In
      </v-btn>
    </div>
